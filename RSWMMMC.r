## Derived from RSWMM, Developed by Peter Steinberg of Herrera Env.
...
runMC <- function(iters, controlList, parT) {
    ...
    for (sim in 1:iters) {
        ...
        ## Call function to sub in parameter values
...
writeNewInputFile <- function(SWMMTemplate, filename) {
    ## Writes a new input file after replacement codes have been replaced
    ##   by parameters
    SWMMTemplate <- gsub("//", "////", SWMMTemplate, ignore.case=FALSE,
...
interpCalDataToSWMMTimes <- function(headObj) {
    ## This linearly interpolates observations for calibration onto the time
    ##   stamps of the SWMM model
    calData$interpedObs <<- approx(calData$times, calData$obs,
                                   headObj$SWMMTimes, method="linear",
...
getCalDataFromCSV<-function(CSVFile, dateFormat="%Y-%m-%d %H:%M") {
    ...
    ## The dateFormat is passed to strptime, so look for formatting
    ##   information there
    ## For example, dates like this 1/1/07 12:00, can be read with the
    ##   default dateFormat
...
getParmeterBounds <- function(parameterBoundsFile) {
    ## parameterBoundsFile should be a full filename and path of a CSV
    ##   with 4 columns and one header row.  The format is as follows (but
    ##   do not put the comment marker # in there):
    ##     Code,Minimum,Maximum,Initial
    ##     $1$,0,1,.5
    ##     $2$,1,2,1.5
    ## The codes are the dollar sign enclosed codes you used in the template
...
## Gets a single SWMM result.  See the documentation in SWMM intefacing for
##   detail
...
## iIndex is the position of the subcatch, node, or link among the other
##   subcatch, links, nodes
## I would recommend using getSWMMTimeSeries so that you don't have to know
##   iIndex.  That function is a wrapper around this one and looks up
##   results based on names not indicies.
## 1/24/2012 edit: Fixed bug in returning results for models with pollutants
    ...
    if (iType==SUBCATCH) {
        ...
        off <- off+ RECORDSIZE*(headObj$numSubc*(headObj$numSubcVars) +
                               headObj$numNode*(headObj$numNodeVars) +
                               headObj$numLink*(headObj$numLinkVars) +
                               vIndex)
    }
...
getSWMMTimeSeriesData <- function(headObj, iType, nameInOutputFile, vIndex) {
    ...
    ## nameInOutputFile should be the exact name in the output file of a
    ##   subcatchment, link, or node, or leave this as an empty string if
    ##   searching for system results
    ## vIndex should be selected from this lists below,depending on whether
    ##   iType is a subcatchment, link or node.
## ###########################################################
...
## ###########################################################
    ...
    if(iType==0) {
        iIndex <-
            (0:(headObj$numSubc-1))[headObj$subcNames==nameInOutputFile]
    } else if(iType==1) {
        iIndex <-
            (0:(headObj$numNode-1))[headObj$nodeNames==nameInOutputFile]
    } else if(iType==2) {
        iIndex <-
            (0:(headObj$numLink-1))[headObj$linkNames==nameInOutputFile]
    } else if(iType==3) {
        iIndex <- 0
    }
    ...
    for(period in 0:(-1+headObj$numReportingPeriods)) {
        ## browser()
        output[period+1] <- getSWMMResult(headObj=headObj, iType=iType,
                                          iIndex=iIndex, vIndex=vIndex,
                                          period=period)
    }
    return(output)
}
...
objectiveFunction <- function(x, performanceStat,
                              functionCallToEvalForASWMMTimeSeries,
                              baseOutputName, SWMMTemplateFile, SWMMexe) {
    ## A number of global variables must be defined before this function is
    ##   called iteration should be set to 1 in the global namespace
    ## parametersTable should be created in the global namespace by
    ##   calling getParameterBounds
    ## SWMMTemplate should be created in the global namespace
    ## calDataObj should exist in the global namespace after calling
    ##   calDataObj=readTemplateFile(...)
    ## optimizationHistoryFile should exist in the global namespace after
    ##   calling
    ##   optimizationHistoryFile=openOptimizationHistoryFile(filename) on
    ##   a given filename
...
##    browser()
    if(iteration==1) {
        ...
        optFileHeader <- c("Iteration",paste("Parameter",
                                             t(parametersTable["Code"]),
                                             sep=""), performanceStat)
    }
    ...
    codeToMakeThreadSafe <- paste(
        ...
        sep="", collapse="")
    inpFile <- paste(baseOutputName, iteration, " ",
                     codeToMakeThreadSafe, '.inp', sep="")
    rptFile <- paste(baseOutputName, iteration, " ", codeToMakeThreadSafe,
                  '.rpt', sep="")
    outFile <- paste(baseOutputName, iteration, " ", codeToMakeThreadSafe,
                     '.out', sep="")
    writeNewInputFile(SWMMTemplateModified, inpFile)
    ...
    if(errCode==0) {
        headObj <- openSWMMOut(outFile)
        ## removed the returnObjectProperties argment on 2/7/2011
        headObj <- getSWMMTimes(headObj)
        interpCalDataToSWMMTimes(headObj)
        correspondingSWMMSeries <-
            eval(parse(text=functionCallToEvalForASWMMTimeSeries))
        ...
        summaryRow <- unlist(c(iteration,x,perfStatsToUse))
        stop(paste("SWMM returned an error. Optimization stopping.  See",
                   rptFile))
        return(array(NaN, length(performanceStat)))
    }
    ...
    ## browser()
    save(optimizationHistory, file=paste(baseOutputName,
                                         'RBinaryOptimizationHistory',
                                         '.rData',sep=""))
...
performanceStatsAsMinimization <- function(correspondingSWMMSeries,
                                           includeOnlyNonNAN=TRUE) {
    ## calDataObj should have the fields times, obs, and interpedObs, which
    ##   can be obtained by calling getCalDataFromCSV and
    ##   interpCalDataToSWMMTimes
    ## headObj should be a header returned by openSWMMOut and modified by
    ##   getSWMMTimes
    ## CorrespondingSWMMSeries should be a series of just data points for
    ##   comparison to calDataObj$interpedObs.  It could be obtained by
    ##   calling getSWMMTimeSeriesData.  It should have the same length as
    ##   calDataObj$interpedObs
    ## All performance stats given are expressed so that they can be
    ##   minimized to maximize the fit of the model to the data.  For
    ##   example, correlation is mulitplied by -1, so that minimizing it
    ##   improves correlation
    ## 2/21/2012 edit: added includeOnlyNonNAN argument to restrict the
    ##   calculation to only non-NAN values
    ...
    linearCorrelationTimesMinus1 <-
        -1*cor(calData$interpedObs[!is.nan(calData$interpedObs)],
               correspondingSWMMSeries[!is.nan(calData$interpedObs)])
    mean4thPowerError <- mean(residualz^4)
    output <- data.frame(meanAbsoluteError, sumOfSquaredError,
                         linearCorrelationTimesMinus1 ,mean4thPowerError)
    ## browser()
    return(output)
}
...
readLID <- function(filename,headObj) {
    ...
    ## headObj should be obtained by calling both openSWMMOut and
    ##   getSWMMTimes
    ## filename is the LID report filename
    ## Skips the first 9 header lines and reads an LID file
    ## Elapsed  Total  Total  Surface  Soil	  Bottom  Surface  Drain
    ##  Time   Inflow  Evap	  Infil	   Perc	  Infil	  Runoff   Outflow
    ##  Hour    in/hr  in/hr  in/hr	   in/hr  in/hr	   in/hr    in/hr
    ## ....
    ##  Surface   Soil/  Storage
    ##  Depth     Pave    Depth
    ##  inches    Moist   inches
    r <- read.table(filename, skip=9, sep='\t')
    names(r) <- c("ElapsedHours", "TotalInflow", "TotalEvap", "SurfaceInfil",
                  "SoilPerc", "BottomInfil", "SurfaceRunoff", "DrainOutflow",
                  "SurfaceDepth","SoilMoist","StorageDepth")
